...
## 9. Comparison with Kani

| Property | Kani | Creusot |
|---|---|---|
| Proof style | Bounded model checking | Deductive (SMT) |
| Coverage | All variants × bounded depth | All inputs simultaneously |
| String fields | `String::new()` (bounded) | `extern_spec!` model required |
| Async code | Not supported | Not supported |
| Proof artefact | `proof_for_contract` harness | COMA + Why3 session |
| Failure output | CBMC counterexample | Unproved goal + COMA position |
| Composition | `stub_verified` | Contracts inline |
| Turnaround | ~30s per harness | ~1–2min for full suite |
| Companion source | `src/kani/generated/*.rs` | `src/creusot/generated/*.rs` |
| Both generated by | `build.rs` | same |

The two backends are complementary. Kani finds concrete counterexamples at
bounded depth; Creusot proves universally for all inputs. Running both gives
high confidence in VSM invariant preservation.
