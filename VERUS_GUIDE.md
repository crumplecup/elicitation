...
**Current status: 293 proofs registered, 780 verification goals discharged.**
The proof suite includes substantive arithmetic proofs (ui_types), shadow
struct field-preservation proofs (egui, ratatui composites), boolean stub
proofs (select enums, float-field types), and a 10-level gallery curriculum
for VSM companion development (V1–V10).
...
`crates/elicitation_verus/src/gallery/` is a ten-level curriculum that
validates each Verus capability needed for production VSM companions. Each
level tests one hypothesis and is expected to pass.
...
| V5 | `level5.rs` | `String` fields | `vstd` string model (`s@.len() > 0`) |
...
| V9 | `level9.rs` | `assume_specification` | Trusted axiom for external/async function |
| V10 | `level10.rs` | Proof composition | `Tracked<T>` through two transitions, exec compose |

V7 is the critical level: it proves that a state machine with the exact shape
of a production archive machine can be fully verified without per-variant
harnesses. V9 validates the `assume_specification` mechanism in isolation before
it is used for every archive transition companion.

All ten levels pass: `verification results:: 780 verified, 0 errors`.

---

## Verified State Machine Companions

Verus is applied to the `VerifiedStateMachine` architecture via
`assume_specification` companion contracts generated in `elicit_proofs`.
For the full design, problems encountered, and extension guide, see:

**[`VERUS_FOR_VSMS.md`](VERUS_FOR_VSMS.md)**

Summary:

- Each production VSM transition gets an `assume_specification` block in
  `crates/elicit_proofs/src/verus/generated/<machine>.rs`
- All blocks are gated with `#[cfg(verus)]` — invisible to normal Rust builds
- Companion files are auto-generated by `cargo build -p elicit_proofs`
- The invariant `pub open spec fn` is embedded inline (not imported) to avoid
  cross-crate spec-body opacity
- Run `just verify-verus` (gallery) and `just check elicit_proofs` (companions)
...
| Verification goals | 780 |
| Shadow struct types | 5 |
| Gallery levels | 10 (V1–V10) |
| Modules | 9 + gallery |
