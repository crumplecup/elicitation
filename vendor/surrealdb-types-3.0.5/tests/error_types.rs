...
fn test_public_error_validation_details() {
    let err = Error::validation(
        ...
        Some(ValidationError::InvalidRequest),
    );
    assert!(err.is_validation());
    assert_eq!(
        err.validation_details(),
        Some(&ValidationError::InvalidRequest)
    );
...
fn test_public_error_details_pattern_matching() {
    ...
    match err.details() {
        ErrorDetails::NotFound(Some(NotFoundError::Table { name })) => assert_eq!(name, "users"),
...
fn test_detail_wire_format_unit_variant() {
    ...
    assert!(
        ...
        "Unit variant should not have details field"
    );
...
fn test_detail_wire_format_newtype_variant() {
    ...
    assert_eq!(
        inner.get("kind"),
        Some(&Value::String("TokenExpired".into()))
    );
    assert!(
        !inner.contains_key("details"),
        "Inner unit variant should not have details"
    );
...
fn test_detail_wire_format_newtype_with_struct_inner() {
    ...
    assert_eq!(
        inner_details.get("actor"),
        Some(&Value::String("user:john".into()))
    );
    ...
    assert!(matches!(
        parsed,
        NotAllowedError::Auth(AuthError::NotAllowed { .. })
    ));
}
...
fn test_detail_wire_format_all_flat_enums() {
    ...
    let val = ValidationError::InvalidParameter { name: "x".into() }.into_value();
    let parsed = ValidationError::from_value(val).unwrap();
    assert_eq!(
        parsed,
        ValidationError::InvalidParameter { name: "x".into() }
    );
...
fn test_detail_wire_format_query_timeout() {
    ...
    assert!(
        obj.contains_key("details"),
        "Struct variant should have details field"
    );
...
fn test_error_snapshot_not_allowed_auth_token_expired() {
    ...
    assert_eq!(
        obj.get("message"),
        Some(&Value::String("Token expired".into()))
    );
    ...
    assert!(
        !inner.contains_key("details"),
        "Unit variant should not have inner details key"
    );
...
fn test_error_snapshot_query_timed_out() {
    ...
    assert!(
        details.contains_key("details"),
        "Struct variant should have details"
    );
...
fn test_error_snapshot_already_exists_record() {
    ...
    assert_eq!(
        record_details.get("id"),
        Some(&Value::String("users:123".into()))
    );
...
fn test_error_snapshot_not_allowed_method() {
    ...
    assert_eq!(
        method_details.get("name"),
        Some(&Value::String("begin".into()))
    );
...
fn test_error_snapshot_thrown_no_details() {
    ...
    assert_eq!(
        obj.get("message"),
        Some(&Value::String("custom error".into()))
    );
...
fn test_error_snapshot_not_allowed_scripting_unit() {
    ...
    // }
    let err = Error::not_allowed(
        "Scripting not allowed".to_string(),
    ...
    assert_eq!(
        details.get("kind"),
        Some(&Value::String("Scripting".into()))
    );
    assert!(
        !details.contains_key("details"),
        "Unit variant should not have inner details"
    );
...
fn test_error_wire_validation_parse() {
    ...
    assert_eq!(
        obj.get("message"),
        Some(&Value::String("Failed to parse query".into()))
    );
    assert_eq!(obj.get("kind"), Some(&Value::String("Validation".into())));
    let Some(Value::Object(details)) = obj.get("details") else {
        panic!("Expected details");
    ...
    assert!(
        !details.contains_key("details"),
        "Unit variant has no inner details"
    );
...
fn test_error_wire_validation_invalid_parameter_with_name() {
    ...
    assert_eq!(
        details.get("kind"),
        Some(&Value::String("InvalidParameter".into()))
    );
    let Some(Value::Object(inner)) = details.get("details") else {
        panic!();
    };
...
fn test_error_wire_not_allowed_auth_session_expired() {
    ...
    assert_eq!(
        auth.get("kind"),
        Some(&Value::String("SessionExpired".into()))
    );
...
fn test_error_wire_not_allowed_function() {
    ...
    assert!(parsed.is_not_allowed());
    match parsed.not_allowed_details() {
        Some(NotAllowedError::Function { name }) => assert_eq!(name, "fn::custom"),
...
fn test_error_wire_not_allowed_target() {
    ...
    assert!(parsed.is_not_allowed());
    match parsed.not_allowed_details() {
        Some(NotAllowedError::Target { name }) => assert_eq!(name, "example.com"),
...
fn test_error_wire_not_found_record() {
    ...
    match parsed.not_found_details() {
        Some(NotFoundError::Record { id }) => assert_eq!(id, "person:jane"),
...
fn test_error_wire_not_found_namespace() {
    ...
    assert!(parsed.is_not_found());
    match parsed.not_found_details() {
        Some(NotFoundError::Namespace { name }) => assert_eq!(name, "test_ns"),
...
fn test_error_wire_not_found_database() {
    ...
    assert!(parsed.is_not_found());
    match parsed.not_found_details() {
        Some(NotFoundError::Database { name }) => assert_eq!(name, "test_db"),
...
fn test_error_wire_not_found_method() {
    ...
    assert!(parsed.is_not_found());
    match parsed.not_found_details() {
        Some(NotFoundError::Method { name }) => assert_eq!(name, "unknown_method"),
...
fn test_error_wire_not_found_transaction() {
    ...
    assert!(parsed.is_not_found());
    assert_eq!(
        parsed.not_found_details(),
        Some(&NotFoundError::Transaction)
    );
}
...
fn test_error_wire_not_found_session() {
    ...
    assert!(parsed.is_not_found());
    match parsed.not_found_details() {
        Some(NotFoundError::Session { id }) => assert_eq!(id, &Some("abc-123".to_string())),
...
fn test_error_wire_query_timed_out() {
    ...
    match parsed.query_details() {
        Some(QueryError::TimedOut { duration }) => assert_eq!(*duration, Duration::from_secs(30)),
...
fn test_error_wire_already_exists_table() {
    ...
    match parsed.already_exists_details() {
        Some(AlreadyExistsError::Table { name }) => assert_eq!(name, "users"),
...
fn test_error_wire_already_exists_namespace() {
    ...
    assert!(parsed.is_already_exists());
    match parsed.already_exists_details() {
        Some(AlreadyExistsError::Namespace { name }) => assert_eq!(name, "prod"),
        other => panic!("Expected Namespace, got {other:?}"),
    }
}
...
fn test_error_wire_connection_uninitialised() {
    ...
    let err = Error::connection(
        "Connection not initialised".into(),
...
fn test_error_wire_thrown() {
    ...
    assert_eq!(
        parsed.message(),
        "An error occurred: user validation failed"
    );
}
...
fn test_error_from_parts_with_details() {
    ...
    match err.not_found_details() {
        Some(NotFoundError::Table { name }) => assert_eq!(name, "users"),
        other => panic!("Expected Table, got {other:?}"),
    }
}
...
fn test_error_cause_chain_three_levels() {
    ...
    assert_eq!(
        parsed.cause().unwrap().cause().unwrap().message(),
        "record missing"
    );
}
