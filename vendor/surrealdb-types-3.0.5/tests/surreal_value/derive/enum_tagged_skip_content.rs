...
fn test_skip_content_single_missing_content_uses_default() {
    ...
    assert_eq!(
        parsed,
        TestSkipContentWithDefault::Single(String::default())
    );
}
...
fn test_skip_content_option_some_roundtrip() {
    ...
    assert_eq!(
        parsed,
        TestSkipContentOption::WithInner(Some("hello".into()))
    );
}
...
fn test_override_always_skipped_with_some() {
    ...
    assert_eq!(
        obj.get("kind"),
        Some(&Value::String("AlwaysSkipped".into()))
    );
    assert!(!obj.contains_key("details"));
}
...
fn test_override_conditionally_skipped_with_some() {
    ...
    assert_eq!(
        obj.get("kind"),
        Some(&Value::String("ConditionallySkipped".into()))
    );
