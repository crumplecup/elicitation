...
fn contextual_tools_appear_in_list_tools() {
    ...
    assert!(
        !names.contains(&"bet__preset_500"),
        "preset 500 should be filtered"
    );
}
...
fn contextual_re_registration_replaces_tools() {
    ...
    assert!(
        names.contains(&"bet__preset_50"),
        "preset 50 should be present"
    );
    assert!(
        !names.contains(&"bet__preset_100"),
        "preset 100 should be filtered after re-registration"
    );
