...
fn call_formal_produces_correct_output() {
    fn process(
        n: u32,
        _proof: Established<Validated>,
    ) -> (u32, Established<Normalised>) {
...
fn formal_methods_compose_proof_flows_through() {
    ...
    let (normalised_val, normalised_proof) = normalise.call_formal("  hello  ".to_string(), validated);
