...
impl ProofModule {
    ...
    /// Run Prusti verification for the entire crate (all modules).
    ///
    /// Prusti verifies all functions with contracts in a single compilation.
    /// We measure the time and capture any verification failures.
    #[tracing::instrument(skip(self))]
    fn run(&self, timeout: u64) -> Result<ProofResult> {
        tracing::debug!(
            module = %self.name,
        ...
        // Run cargo prusti on the entire crate
        ...
        if duration_secs >= timeout {
            return Ok(ProofResult {
                ...
                duration_secs,
                timestamp,
                error_message: Some(format!("Verification exceeded {}s timeout", timeout)),
            });
        }
        ...
        let status = if output.status.success() && !stderr.contains("error") {
            ProofStatus::Success
        } else {
            ProofStatus::Failed
        };

        let error_message = if status == ProofStatus::Failed {
            ...
            let error_lines: Vec<&str> = stderr
                ...
                .filter(|line| {
                    line.contains("error")
                        ...
                        || line.contains("could not verify")
                })
                .take(10)
                .collect();

            if error_lines.is_empty() {
                Some("Verification failed (see logs)".to_string())
            } else {
