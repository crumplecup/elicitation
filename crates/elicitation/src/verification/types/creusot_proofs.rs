...
//!
//! This module provides formal verification using Creusot, a deductive verifier
//! for Rust based on WhyML and the Why3 platform. Creusot uses SMT solvers
//! (like Z3, CVC4, Alt-Ergo) for automated verification.
//!
...
//!
//! # Creusot Specification Language
//!
//! Creusot uses WhyML-style specifications:
//! - `#[requires(...)]` - Preconditions
//! - `#[ensures(...)]` - Postconditions
//! - `#[logic]` - Pure logical functions
//! - `#[trusted]` - Trusted external code
//! - `proof_assert!` - Assertions in proofs
//!
//! # Example Proof
//!
//! ```rust
...
//!
//! To verify these proofs with Creusot:
//!
//! ```bash
//! # Verify single function
//! cargo creusot --function verify_i8_positive_valid
//!
//! # Verify all proofs in this module
//! cargo creusot --crate elicitation --module verification::types::creusot_proofs
//!
//! # Generate Why3 session
//! cargo creusot --export-why3
//! ```
...
#[cfg(feature = "verification")]
#[ensures(forall<variant: E> result == variant)]
...
#[cfg(all(feature = "verification", feature = "url"))]
...
#[requires(value.starts_with("https://"))]
#[ensures(match ^result {
    Ok(wrapped) => wrapped.into_inner().as_str() == value,
    Err(_) => false,
})]
...
pub const CREUSOT_PROOF_COUNT: usize = 94;
