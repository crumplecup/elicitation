...
pub fn generate_proof_crate(
    ...
    write_file(
        out_dir,
        "Cargo.toml",
        &render_cargo_toml(CargoTomlParams {
            proof_name: &proof_name,
            primary_source: &sources[0].name,
            source_dep_lines: &source_dep_lines,
            elicitation_rel: elicitation_rel.as_deref(),
            elicitation_version: elicitation_version.as_deref(),
            elicitation_preserved: elicitation_preserved.as_deref(),
            creusot_std_rel: creusot_std_rel.as_deref(),
            workspace_pkg_fields: &workspace_pkg_fields,
            workspace_deps: &workspace_deps,
            source_version: source_version.as_deref(),
            extra_workspace_deps: &extra_workspace_deps,
        }),
    )?;
...
struct CargoTomlParams<'a> {
    proof_name: &'a str,
    primary_source: &'a str,
    source_dep_lines: &'a [&'a str],
    elicitation_rel: Option<&'a Path>,
    elicitation_version: Option<&'a str>,
    elicitation_preserved: Option<&'a str>,
    creusot_std_rel: Option<&'a Path>,
    workspace_pkg_fields: &'a [String],
    workspace_deps: &'a [String],
    source_version: Option<&'a str>,
    extra_workspace_deps: &'a [String],
}
    ...
    for vsm in vsms {
        ...
        for item in &syntax.items {
            if let syn::Item::Use(u) = item
                && let Some(root) = use_tree_root_crate(&u.tree)
            {
