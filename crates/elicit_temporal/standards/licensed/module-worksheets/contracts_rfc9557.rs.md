...
| `4.2 Examples` | `informative` | example strings in `RFC 9557 §4.2` instantiate `IxdtfTimestampEvidence`, `ZonedTimestampEvidence`, and `IxdtfCalendarAwareTimestampEvidence` families already classified elsewhere | Examples do not introduce independent normative propositions beyond the syntax and zone semantics already tracked in this worksheet set. |
...
| `7.1 Excessive Disclosure` | `operational guidance` | no dedicated proposition; disclosure-minimization remains deployment policy, not a temporal contract law | Keep privacy and disclosure minimization out of the lexical contract surface unless the framework later introduces an explicit redaction or disclosure-policy accord. |
| `7.2 Data Format Implementation Vulnerabilities` | `operational guidance` | parser hardening and implementation review, not a temporal contract proposition | Keep generic parser-vulnerability guidance out of the proposition surface; it belongs to implementation quality controls. |
| `7.3 Operating with Inconsistent Data` | `covered elsewhere` | `IxdtfCriticalSuffixTagsRequireProcessingOrErrorHandling`, `CriticalTimeZoneSuffixInconsistencyRequiresAction`, `ElectiveTimeZoneSuffixInconsistencyMayBeHandled`, `ZoneTransitionDisambiguationAuthorityDeclared`, `ZoneTransitionGapHandlingAuthorityDeclared` | The inconsistent-data theme is already represented where RFC 9557 assigns concrete behavior; the broader security framing remains operational guidance rather than an extra lexical law. |
