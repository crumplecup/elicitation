...
| `format_qualified_temporal_value` | `covered` | `format_qualified_temporal_value`, `FormattedQualifiedTemporalValueResult`, `QualifiedTemporalExpressionEvidence`, `QualificationPlacementEvidence`, `QualifiedTemporalValueEvidence` | The formatter seam now requires and re-issues the qualified-value proof, the qualification-expression proof, and the placement-family branch explicitly rather than flattening qualification law into one coarse validity token. |
| `format_grouped_time_scale_unit` | `covered` | `format_grouped_time_scale_unit`, `FormattedGroupedTimeScaleUnitResult`, `GroupedTimeScaleUnitEvidence` | The formatter seam now requires and re-issues delimiter, unit-carriage, continuity, coefficient, bounds, explicit-time-shift, truncation, and interval-conversion sidecars explicitly rather than flattening grouped-unit law into one aggregate validity token. |
| `format_seasonal_temporal_expression` | `covered` | `format_seasonal_temporal_expression`, `FormattedSeasonalTemporalExpressionResult`, `SeasonalTemporalExpressionEvidence` | The formatter seam now requires and re-issues year-and-season form, month-slot encoding, named-season declaration, and season-scope declaration explicitly rather than flattening seasonal law into coarse validity alone. |
| `format_sub_year_grouping_expression` | `covered` | `format_sub_year_grouping_expression`, `FormattedSubYearGroupingExpressionResult`, `SubYearGroupingExpressionEvidence`, `SubYearGroupingKindEvidence` | The formatter seam now requires and re-issues year-and-grouping form, month-slot encoding, and the grouping-family branch explicitly, preserving quarter, quadrimester, semestral, and seasonal distinctions. |
| `format_unspecified_component_expression` | `covered` | `format_unspecified_component_expression`, `FormattedUnspecifiedComponentExpressionResult`, `UnspecifiedComponentExpressionEvidence` | The formatter seam now requires and re-issues placeholder syntax, unknown-value semantics, Level 1 right-tail masking, and Level 2 internal-component masking explicitly rather than flattening masking law into coarse validity alone. |
