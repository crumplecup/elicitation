...
#[derive(Prop)]
#[prop(credential = WcagVerified, creusot_invariant_fn = "archive_nav_consistent", kani_invariant_fn = "archive_nav_consistent", verus_invariant_fn = "archive_nav_consistent", verus_inv_body = "match *state { ArchiveNavState::NavFiltered { filter, .. } => filter@.len() > 0, _ => true, }")]
