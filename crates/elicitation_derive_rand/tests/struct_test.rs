...
fn test_server_config() {
    ...
    for _ in 0..20 {
        ...
        // Port in range
        assert!(config.port >= 8000 && config.port < 9000, 
                "Port {} out of range [8000, 9000)", config.port);
        
        // Timeout positive and even
        assert!(config.timeout_ms > 0, "Timeout not positive: {}", config.timeout_ms);
        assert_eq!(config.timeout_ms % 2, 0, "Timeout not even: {}", config.timeout_ms);
...
fn test_point() {
    ...
    for _ in 0..20 {
        ...
        assert!(point.x >= -100 && point.x < 100, "x {} out of bounds", point.x);
        assert!(point.y >= -100 && point.y < 100, "y {} out of bounds", point.y);
    }
}
...
fn test_game_settings() {
    ...
    for _ in 0..20 {
        ...
        assert!(settings.dice_sides >= 1 && settings.dice_sides < 6, 
                "Dice sides {} out of range", settings.dice_sides);
        assert_eq!(settings.player_count % 2, 1, 
                   "Player count {} not odd", settings.player_count);
        assert_eq!(settings.npc_count % 2, 0, 
                   "NPC count {} not even", settings.npc_count);
    }
}
