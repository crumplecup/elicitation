...
fn test_unit_enum() {
    ...
    assert!(has_active || has_inactive || has_pending, "Should generate at least one variant");
}
...
fn test_tuple_enum() {
    ...
    for _ in 0..20 {
        ...
        match result {
            GameResult::Win(score) => { let _ = score; }
            GameResult::Loss(damage) => { let _ = damage; }
            GameResult::Draw => {}
        }
    }
}
...
fn test_struct_enum_with_contracts() {
    ...
    for _ in 0..20 {
        ...
        match action {
            GameAction::Move { x, y } => {
                assert!(x >= -10 && x < 10, "x {} out of bounds", x);
                assert!(y >= -10 && y < 10, "y {} out of bounds", y);
            }
            GameAction::Attack { damage } => {
                assert!(damage >= 1 && damage < 100, "damage {} out of bounds", damage);
            }
            GameAction::Defend => {}
        }
    }
...
fn test_mixed_enum() {
    ...
    for _ in 0..20 {
        ...
        match event {
            ...
            Event::KeyPress(c) => { let _ = c; }
            Event::Scroll(delta) => { let _ = delta; }
            Event::Quit => {}
        }
    }
}
