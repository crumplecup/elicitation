...
impl Paragraph {
    ...
    pub fn wrap(&self, trim: bool) -> Paragraph {
        Paragraph(std::sync::Arc::new(
            (*self.0).clone().wrap(ratatui::widgets::Wrap { trim }),
        ))
    }
    ...
    pub fn scroll(&self, vertical: u16, horizontal: u16) -> Paragraph {
        Paragraph(std::sync::Arc::new(
            (*self.0).clone().scroll((vertical, horizontal)),
        ))
    }
}
...
impl<'de> serde::Deserialize<'de> for Paragraph {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        struct ParagraphJson {
            text: Option<crate::Text>,
        }
        let ParagraphJson { text } = ParagraphJson::deserialize(d)?;
        let rt = text.map(|t| (*t.0).clone()).unwrap_or_default();
        Ok(Paragraph(std::sync::Arc::new(
            ratatui::widgets::Paragraph::new(rt),
        )))
    }
}
mod emit_impls {
