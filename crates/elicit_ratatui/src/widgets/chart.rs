...
impl Chart {
    ...
    pub fn block(&self, block: crate::Block) -> Chart {
        Chart(std::sync::Arc::new((*self.0).clone().block((*block.0).clone())))
    }

    /// Set the x-axis.
    #[tracing::instrument(skip(self, axis))]
    pub fn x_axis(&self, axis: crate::Axis) -> Chart {
        Chart(std::sync::Arc::new((*self.0).clone().x_axis((*axis.0).clone())))
    }

    /// Set the y-axis.
    #[tracing::instrument(skip(self, axis))]
    pub fn y_axis(&self, axis: crate::Axis) -> Chart {
        Chart(std::sync::Arc::new((*self.0).clone().y_axis((*axis.0).clone())))
    }
...
impl<'de> serde::Deserialize<'de> for Chart {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        struct ChartJson { #[serde(default)] datasets: Vec<crate::Dataset> }
        ...
        Ok(Chart(std::sync::Arc::new(ratatui::widgets::Chart::new(ratatui_datasets))))
    }
}
mod emit_impls {
