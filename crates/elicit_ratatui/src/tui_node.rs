//! Internal AccessKit bridge types.
//!
//! `TuiNode` and `WidgetJson` are the internal representation used by
//! `render_backend` and `tui_accesskit_convert`.  These are NOT the public
//! MCP shadow types — those live in `primitives`, `style`, `text`, `layout`,
//! `widgets`.  This module will be removed once `render_backend` is migrated
//! to use the shadow types directly.
