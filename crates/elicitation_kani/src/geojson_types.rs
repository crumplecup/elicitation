...
fn verify_geojson_value_point_type_name() {
    let value = geojson::Value::Point(vec![1.0_f64, 2.0_f64]);
...
fn verify_geojson_geometry_new_point() {
    let geometry = geojson::Geometry::new(geojson::Value::Point(vec![3.0_f64, 4.0_f64]));
    assert!(
...
fn verify_geojson_feature_property_access() {
    let mut feature = geojson::Feature::from(geojson::Value::Point(vec![5.0_f64, 6.0_f64]));
...
fn verify_geojson_feature_collection_len() {
    let first = geojson::Feature::from(geojson::Value::Point(vec![0.0_f64, 0.0_f64]));
    let second = geojson::Feature::from(geojson::Value::Point(vec![1.0_f64, 1.0_f64]));
