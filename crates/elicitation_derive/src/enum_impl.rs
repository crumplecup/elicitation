...
pub fn expand_enum_e2(input: DeriveInput) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
    let mod_name = quote::format_ident!("_elicit_compat_{}", name);
    let piece = generate_prompt_impl(
        &name,
        custom_prompt,
        &impl_generics,
        &ty_generics,
...
pub fn expand_enum_e3(input: DeriveInput) -> TokenStream {
    ...
    let mod_name = quote::format_ident!("_elicit_compat_{}", name);
    let piece = generate_select_impl(
        &name,
        &unit_refs,
        &labels,
        &impl_generics,
...
pub fn expand_enum_e4(input: DeriveInput) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
    let mod_name = quote::format_ident!("_elicit_compat_{}", name);
    let piece = generate_elicit_impl(
        &name,
        &variants,
        &impl_generics,
        &ty_generics,
...
pub fn expand_enum_e5(input: DeriveInput) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
    let mod_name = quote::format_ident!("_elicit_compat_{}", name);
    let piece = generate_introspect_impl(
        &name,
        &variants,
        &impl_generics,
        &ty_generics,
...
pub fn expand_enum_e7(input: DeriveInput) -> TokenStream {
    ...
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
    let mod_name = quote::format_ident!("_elicit_compat_{}", name);
    let piece = generate_prompt_tree_impl_enum(
        &name,
        &variants,
        custom_prompt.as_deref(),
        &impl_generics,
...
pub fn expand_enum_e9(input: DeriveInput) -> TokenStream {
    ...
    let prompt = generate_prompt_impl(
        &name,
        custom_prompt,
        &impl_generics,
        &ty_generics,
    ...
    let select = generate_select_impl(
        &name,
        &unit_refs,
        &labels,
        &impl_generics,
    ...
    let elicit = generate_elicit_impl(
        &name,
        &variants,
        &impl_generics,
        &ty_generics,
...
pub fn expand_enum_e10(input: DeriveInput) -> TokenStream {
    ...
    let introspect = generate_introspect_impl(
        &name,
        &variants,
        &impl_generics,
        &ty_generics,
...
pub fn expand_enum_e12(input: DeriveInput) -> TokenStream {
    ...
    let prompt_tree = generate_prompt_tree_impl_enum(
        &name,
        &variants,
        custom_prompt.as_deref(),
        &impl_generics,
...
pub fn expand_enum_e13(input: DeriveInput) -> TokenStream {
    let (name, generics, variants, unit_idents, labels, custom_prompt) =
        bisect_parse(input.clone());
    let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
    let unit_refs: Vec<&syn::Ident> = unit_idents.iter().collect();
