...
use crate::rand_contract_parser::{Contract, parse_contract};
...
pub fn expand_derive_rand(input: &DeriveInput) -> Result<TokenStream> {
    ...
    match &input.data {
        Data::Struct(data_struct) => generate_struct_impl(input, data_struct, contract),
        Data::Enum(data_enum) => generate_enum_impl(input, data_enum, contract),
...
fn generate_newtype_impl(
    ...
    let generator_expr = match contract {
        ...
        Some(Contract::Positive) => generate_positive_impl(inner_type)?,
        Some(Contract::NonZero) => generate_nonzero_impl(inner_type)?,
        Some(Contract::Even) => generate_even_impl(inner_type)?,
        Some(Contract::Odd) => generate_odd_impl(inner_type)?,
        Some(Contract::And(left, right)) => generate_and_impl(inner_type, &left, &right)?,
        Some(Contract::Or(left, right)) => generate_or_impl(inner_type, &left, &right)?,
        None => {
...
fn generate_bounded_impl(inner_type: &syn::Type, low: &Lit, high: &Lit) -> Result<TokenStream> {
    Ok(quote! {
    ...
    let field_inits: Vec<TokenStream> = fields
        ...
        .map(|field| {
            ...
            let generator_expr = match field_contract {
                ...
                Some(Contract::Positive) => generate_positive_impl(field_type)?,
                Some(Contract::NonZero) => generate_nonzero_impl(field_type)?,
                Some(Contract::Even) => generate_even_impl(field_type)?,
                Some(Contract::Odd) => generate_odd_impl(field_type)?,
                Some(Contract::And(left, right)) => generate_and_impl(field_type, &left, &right)?,
                Some(Contract::Or(left, right)) => generate_or_impl(field_type, &left, &right)?,
                None => {
...
fn generate_enum_impl(
    ...
    if variant_count == 0 {
        return Err(syn::Error::new_spanned(
            input,
            "Cannot derive Rand for empty enum",
        ));
    }
                ...
                Fields::Unnamed(fields) => {
                    let field_gens: Vec<TokenStream> = fields
                        ...
                        .map(|field| {
                            ...
                            let generator_expr =
                                generate_field_generator(field_type, field_contract)?;
...
fn generate_field_generator(
    ...
    match field_contract {
        Some(Contract::Bounded { low, high }) => generate_bounded_impl(field_type, &low, &high),
        Some(Contract::Positive) => generate_positive_impl(field_type),
        Some(Contract::NonZero) => generate_nonzero_impl(field_type),
        Some(Contract::Even) => generate_even_impl(field_type),
        Some(Contract::Odd) => generate_odd_impl(field_type),
